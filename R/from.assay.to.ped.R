#' @title transform the output of the function \code{read.assay.file} into data.ped.
#'
#' @description
#' \code{from.assay.to.ped} transform the output of function \code{read.assay.file} into data.ped 
#' then you can save the data.ped as file.ped. before using this function, you need to prepare the 
#' information of SNP and samples.
#'
#' @details
#' This function use the output of the function \code{read.assay.file}.
#' if the snp.info and sample.info data are available, you should input
#' them, too. The output of this function is a data frame in ped format. 
#' The first 6 col of output are about sample information. And the other 
#' col are the snp from the assay files.
#'
#' @param assay output of the function \code{read.assay.file}.
#' @param snp.info a data frame containing the SNP information include the assay name or rs id, the ref allele, alter allele of each SNP.
#' @param sample.info a data frame containing the sample information include the id, Sex and phenotype of each sample.
#' @param is.sample.info logical. If \code{TRUE}, the sample.info is required and the funtion will merge the sample information to the final output.
#' @param col.names.convert logical. if \code{TRUE}, convert the colnames of assay to the rs id of each SNP, such as: rs001.
#' @param fam.id logical. if \code{TRUE}, the ped data will contain the family data of the samples.
#' @param is.num logical. if \code{TRUE}, the loci data in ped data will be like "1 1", "1 2" or "2 2". if \code{FALSE}, the loci data in ped data will be like "A A","A B" or "B B".
...
#convert the genotype form VIC, FAM or BOTH into the number format or allele format.
